...
enum Tag {
  ...
  BName = 'B - name',
  BValue = 'B - value',
  relationPositiveCorrelation = 'relation - positive correlation',
  relationNegativeCorrelation = 'relation - negative correlation',
  relationOthers = 'relation - others',
}

export default defineComponent({
  name: 'VStructureAssociation',
  props: {
    label: {
  ...
  data() {
    return {
      Tag,
      relationTags: [
        Tag.relationPositiveCorrelation,
        Tag.relationNegativeCorrelation,
        Tag.relationOthers,
      ],
    };
  },
  computed: {
    relationTypeText(): string {
      ...
      if (unique.length === 0) return 'positive correlation | negative correlation | others';
